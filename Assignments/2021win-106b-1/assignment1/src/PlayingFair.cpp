 ...
 * ------------------------------------
 * The function will return a sequence of order following fair rules.
 *
 * @param n: the order of sequence
 * ex>: order 0: A   order 1: AB   order 3: ABBA
 *
 * Base case:
 *    when order is zero, then return the character itself.
 * Recursive body:
 *    aSequenceOfOrder(n) = aSequenceOfOrder(n-1) + bSequenceOfOrder(n-1)
 *
 * Important: what if input number is negative?
 */
string aSequenceOfOrder(int n) {
    if (n < 0) {
        error("Error: illegal input number, negative input is not allowed.");
    }
 ...
 * ------------------------------------
 * Same as aSequenceOfOrder but slightly different in recursive body.
 */

...
Question randomQuestionFrom(Set<Question>& questions) {
    if (questions.isEmpty()) {
        error("There is no stored questions for users.");
    }
    else {
        Question select = questions.first();
...
Map<char, int> scoresFrom(const Map<Question, int>& answers) {
    ...
    for (Question ques : answers) {
        int answer = answers[ques];
        Map<char, int> factors = ques.factors;

        switch (answer) {
        case 5: // strongly agree: double add
            for (char factor : factors) {
                if (scores.containsKey(factor)) {
                    scores[factor] += factors[factor] * 2;
                }
                else {
                    scores[factor] = factors[factor] * 2;
                }
            ...
            break;
        case 4: // agree: normal add
            for (char factor : factors) {
                if (scores.containsKey(factor)) {
                    scores[factor] += factors[factor];
                }
                else {
                    scores[factor] = factors[factor];
                }
        ...
        case 2: // disagree: normal minus
            ...
            break;
        case 1: // strongly disagree: double minus
            for (char factor : factors) {
                if (scores.containsKey(factor)) {
                    scores[factor] -= factors[factor] * 2;
                }
...
Map<char, double> normalize(const Map<char, int>& scores) {
    if (scores.isEmpty()) {
        error("Error: No scores found!");
    }

    double sum = 0;
    for (char factor : scores) {
        sum += scores[factor] * scores[factor];
    }

    if (sum == 0) {
        error("Error: No non-zero value found in the scores!");
    }

    Map<char, double> normalized;
    sum = sqrt(sum);
    for (char factor : scores) {
        normalized[factor] = scores[factor] / sum;
    }
 ...
 * ------------------------------------
 * The function will calculate the cosine similarity of two given records.
 *
 * cosSimilarity = a1*b1 + a2*b2 + ... + an*bn.
 */
double cosineSimilarityOf(const Map<char, double>& lhs,
    ...
    // works for any key, not only for 'OCEAN'
    for (char key : lhs) {
        factors += key;
    }
    for (char key : rhs) {
    ...
    for (char factor : factors) {
        if (!lhs.containsKey(factor)) continue;
        if (!rhs.containsKey(factor)) continue;
        similarity += lhs[factor] * rhs[factor];
    }
...
Person mostSimilarTo(const Map<char, int>& scores, const Set<Person>& people) {
    Person mostSimilarity;
    double maxSimilarity = 0;

    // doublelize
    Map<char, double> scores_d;
    for (char key : scores) {
        scores_d[key] = double(scores[key]);
    }

    for (Person person : people) {
        Map<char, int> peopleScores = person.scores;
        // doublelize
        Map<char, double> peopleScores_d;
        for (char key : scores) {
            peopleScores_d[key] = double(peopleScores[key]);
        }
        double similarity = cosineSimilarityOf(scores_d, peopleScores_d);

        if (similarity > maxSimilarity) {
            mostSimilarity = person;
